...
> ISDK Language: AI 工程领域缺失的那门编程语言。

ISDK Language 是一门完整的高级编程语言，旨在将提示词上下文工程转变为一个健壮的软件工程学科。它将零散的一次性提示转化为结构化、可复用、可继承、可组合、可调用的智能模块（智能体/流程），并以更接近自然语言的语法降低书写与协作成本。它通过在面向开发者的**`Prompt Context`**（源代码）和面向机器的**`Prompt Packet`**（编译后的 AI 输入）之间建立清晰的层次来实现这一目标。

**语言分层结构：**
ISDK 将提示词脚本组织成开发者熟悉的、面向对象的结构：

* **函数 (`lib`)：** 用于特定任务的可复用、可调用的脚本（例如 `@file()`, `@url()`）。
* **类 (`char`, `type`)：** 封装的、可继承的智能体蓝图。这允许您创建基础智能体并对其进行扩展，覆盖其配置和逻辑。
* **应用：** 由多个 ISDK 文件组成的独立项目，拥有清晰的入口点。
**核心工程原则：**
