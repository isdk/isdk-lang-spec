...
`NOBJ` 的设计初衷是🎯 打造一种既像自然语言一样易于人类阅读，又具备 JSON 结构化优势的数据配置格式。
它的目标不是取代 JSON，而是填补一个关键空白：

> 让大语言模型（LLM）更容易理解并生成结构化的输出结果，从而避免复杂的后处理和二次转换。

如果说 AI 擅长理解“人话”，而程序需要的是“结构化的数据”，那么 `NOBJ` 就是这两者之间的一座桥梁。

`NOBJ`作为AI与代码间的「语义粘合剂」，其价值在于建立了自然语言意图到结构化数据的双向无损映射。未来的突破点在于：如何通过编译器技术将自然语言约束（如"必须"）直接编译为运行时验证逻辑，实现「自然语言即规范」的终极目标。
...
2. 复杂列表:
   ...
   ```yaml
   水果列表 has/have/contain/include/有/包(含|括)
     水果 "苹果"
     水果 "香蕉"
   ```
...
```yaml
登录策略配置:
  启用双因素认证 （依赖:短信网关） ← 括号内为关联必填提示
  允许记住设备 → 需同步配置cookie有效期 ← 箭头引导依赖项
```
